...
let play() = 
    ...
    let pat1 = [|1.0; 1.2; 1.3; 1.4; 1.0|] |> Array.map(flute) 
    let pat2 = [|2.0; 2.4;|] |> Array.map(flute)
    let pat3 = [|1.4; 1.3; 1.2; 1.1|] |> Array.map(flute) 
    let pat4 = [|0.5;0.0;0.5;0.8;0.5;0.0;0.5;0.4;|] |> Array.map(flute) 
    let pat5 = [|0.5;1.5;0.5;1.8;0.5;1.5;0.5;1.4;|] |> Array.map(horn) 
